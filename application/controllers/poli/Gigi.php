...
class Gigi extends CI_Controller
{
    ...
    {
        $data['rekam_medis'] = $this->Kunjungan_model->getAllGigi(date('Y-m-d'));
        $data['judul'] = 'Poli gigi';
        $data['cek'] = 'poli_gigi';
        $data['pesan'] = 'Pasien';
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->view('poli/gigi/index', $data);
            ...
            if ($cek_resep != null) {
                ...
                redirect('poli/gigi/index');
            } else {
                $this->session->set_flashdata('flash', 'step2_gagal');
                redirect('poli/gigi/step2/' . $this->input->post('id_rekam_medis'));
            }
        }
    }
    ...
    public function step1($id)
    {
        ...
        if ($data['rekam_medis']['status'] == 3) {
            redirect('poli/gigi/step2/' . $data['rekam_medis']['id_rekam_medis']);
        }
        if ($this->input->post('lab') != 2) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($data['rekam_medis']['status_lab'] == 2 || $data['rekam_medis']['status_lab'] == 3) {
                    $this->load->view('poli/gigi/step1-edit', $data);
                } else {
                    $this->load->view('poli/gigi/step1', $data);
                }
                $this->load->view('templates/footer');
            } else {
    ...
    public function step2($id)
    {
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar');
        $this->load->view('poli/gigi/step2', $data);
        $this->load->view('templates/footer');
    }
}
